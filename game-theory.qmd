...
![Rock-Paper-Scissors](images/clipboard-1451524197.png){fig-align="center" width="50%"}
...
![](images/clipboard-1978614282.png){fig-align="center" width="60%"}
...
![](images/clipboard-501370081.png){fig-align="center" width="50%"}
...
![](images/clipboard-790339449.png){fig-align="center" width="50%"}
...
A strategy $s_{i} \in S_{i}$ is strictly (s.) dominated by $\sigma_{i} \in \Delta\left(S_{i}\right)$ if
...
![$B$ is s. dominated by $1 / 2 T+1 / 2 M$.](images/clipboard-645440440.png){fig-align="center" width="50%"}
...
>
> [*John Maynard Keynes,*\
> *General Theory of Employment, Interest and Money, 1936*]{text-align="right"}
...
::: {#def-idsds}
For all $i \in N$, set $S_{i}^{0}=S_{i}$ and define $S_{i}^{k}$ recursively by
...
A strategy $\sigma_{i} \in S_{i}$ is a best response to a belief $\sigma_{-i} \in \Delta\left(S_{-i}\right)$ if
...
::: {#def-rs}
Set $S^{0}=S$ and let $S^{k}$ be given recursively by
...
::: {#thm-ibr}
For every $k \geq 0$, each $s_{i} \in S_{i}^{k}$ is a best response (within $S_{i}$ ) to a belief in $\Delta\left(S_{-i}^{k-1}\right)$.
:::
...
::: {#cor-2}
If the game is finite, then each $s_{i} \in S_{i}^{\infty}$ is a best response (within $S_{i}$ ) to a belief in $\Delta\left(S_{-i}^{\infty}\right)$.
:::

### Closed under Rational Behavior

::: {#def-crb}
A set $Z=Z_{1} \times \ldots \times Z_{n}$ with $Z_{i} \subseteq S_{i}$ for $i \in N$ is closed under rational behavior if, for all $i$, every strategy in $Z_{i}$ is a best response to a belief in $\Delta\left(Z_{-i}\right)$.
:::
...
An information (or belief) structure is a list $\left(\Omega,\left(l_{i}\right)_{i \in N},\left(p_{i}\right)_{i \in N}\right)$
...
::: {#lem-1}
An event $F$ is common knowledge at $\omega$ iff there exists a public event $F^{\prime}$ with $\omega \in F^{\prime} \subseteq F$.
...
::: {#def-11}
A strategy $s_{i} \in S_{i}$ consistent with common knowledge of rationality if there exists a model $\left(\Omega,\left(l_{j}\right)_{j \in N},\left(p_{j}\right)_{j \in N}, \mathbf{s}\right)$ and state $\omega^{*} \in \Omega$ with $\mathbf{s}_{i}\left(\omega^{*}\right)=s_{i}$ at which it is common knowledge that all players are rational.
...
![Coordination Game, Battle of the Sexes, Matching Pennies](images/clipboard-2785630839.png){fig-align="center" width="80%"}
...
A mixed-strategy profile $\sigma^{*}$ is a Nash equilibrium if for every $i \in N$,
...
![](images/clipboard-102551198.png){fig-align="center" width="80%"}
...
![](images/clipboard-1044758399.png){fig-align="center"}
...
![](images/clipboard-3039300426.png){fig-align="center"}
...
![Courtesy of The MIT Press. Used with permission.](images/clipboard-197530818.png){fig-align="center"}

<br>

![Shapley (1964) Cycling](images/clipboard-1532667264.png){fig-align="center" width="50%"}
...
![Coordination Game, Battle of the Sexes, Matching Pennies](images/clipboard-1158608298.png){width="80%"}
...
![](images/clipboard-849157636.png){fig-align="center"}

::: {style="margin: -30px;"}
:::

![](images/clipboard-2810149651.png){fig-align="center"}

::: callout-important
## Proof

![](images/clipboard-2258422708.png)
:::

Courtesy of John F. Nash, Jr. "[Equilibirum Points in N-Person Games](http://www.pnas.org/content/36/1/48.full)". Proceedings of the National Academy of Sciences of the United States of America 36 no. 1 (1949): 48-49.
...
A correspondence $F: X \rightrightarrows Y$ with compact Hausdorff range $Y$ is closed iff it is upper hemicontinuous and closed-valued.
:::
...
Suppose that $f: X \times Y \rightarrow \mathbb{R}$ is a continuous function, where $X$ and $Y$ are metric spaces and $Y$ is compact.

1.  The function $M: X \rightarrow \mathbb{R}$, defined by
...
2.  The correspondence $F: X \rightrightarrows Y$,
...
Let $X$ be a non-empty, compact, and convex subset of a Euclidean space and let the correspondence $F: X \rightrightarrows X$ have closed graph and non-empty convex values. Then the set of fixed points of $F$ is non-empty and compact.
...
::: {#def-13}
If $X$ is a convex subset of a real vector space, then the function $f: X \rightarrow \mathbb{R}$ is quasi-concave if
...
::: {#thm-9}
Consider a game $(N, S, u)$ such that $S_{i}$ is a convex and compact subset of a Euclidean space and that $u_{i}$ is continuous in $s$ and quasi-concave in $s_{i}$ for all $i \in N$. Then there exists a pure strategy Nash equilibrium.
:::
